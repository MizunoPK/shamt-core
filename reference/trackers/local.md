...
None — the file is the source of truth. `/e1-start-story {id-or-slug}` reads the resolved `stories/{ID}-{slug}-{brief}/ticket.md` (located by ticket ID or slug per §PO-tree resolution) directly and proceeds to the open-questions iterative dialog for any unresolved sections.
