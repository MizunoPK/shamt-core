...
| D2: a PO-tree producer writes the collapsed `epics/{parent-feature-folder}/stories/…` where §PO-tree resolution pins the full nested `epics/{epic-folder}/features/{feature-folder}/stories/…` | Fixing it spans the command↔skill mirror pair (multi-file) and needs judgment on every producer's placeholder vocabulary — capture, not auto-fix. |
