...
1. **Resolve the artifact folder** — at story altitude, resolve the story folder per templates/SHAMT_RULES.template.md §PO-tree resolution (tree-wide glob + legacy-flat fallback; halt on multiple or zero); read `active_artifacts.md` first when present. At framework altitude, the plan path is absolute and lives under `proposals/`; there is no folder glob and no `active_artifacts.md` to read.
