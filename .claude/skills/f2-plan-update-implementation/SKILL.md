...
1. **Read proposal**, confirm validation footer, walk every Proposed Changes row. Note paired-file dependencies (rule ↔ template, command ↔ skill, ref ↔ rule pointer). If any pair is missing from the table, perform an **in-place amendment** (append the missing row, strip the prior footer, re-run `/validate-artifact`; see the command body's In-place amendment section) rather than re-running `/f1-propose-update`.
