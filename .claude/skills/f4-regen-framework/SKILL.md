...
1. **Resolve script path** at `.shamt-core/scripts/regenerate-framework.sh`. Confirm executable.
2. **Resolve target** — `--target <dir>` override or current working directory. State the target path in chat in one line.
3. **Run regen** — `bash .shamt-core/scripts/regenerate-framework.sh --target {dir}`. Surface `wrote` / `removed` / `WARN:` lines. Special-case warnings: unmanaged file collisions, missing `jq`, broken `settings.json`.
4. **Run drift check** — `bash .shamt-core/scripts/regenerate-framework.sh --check --target {dir}`. Expected: `no drift` (exit 0). Any `DRIFT` or `STALE` lines halt Phase 5.
