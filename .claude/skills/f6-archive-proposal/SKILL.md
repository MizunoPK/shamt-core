...
3. **Commit + squash-merge + delete branch** — behind pre-merge guards (HEAD on the proposal branch; working tree holds the canonical edits + archive move + regen output **plus any in-flow audit auto-fixes / captured f0 stubs**, which fold into this landing — `git status` is shown for visibility, not as a halt gate (unrelated tree state — ad-hoc proposals or other in-progress work — is also accepted and folds into the landing); regen has run; branch is a descendant of base). **`git add -A` before committing** — mandatory: the Step 2 `git mv` stages only the rename of the pre-validation blob, leaving the validated content modification (plus the canonical edits, regen output, audit auto-fixes, and f0 captures, including untracked files) unstaged; a partial stage would commit the stale `Status: Draft`, footer-stripped blob. Commit with subject `shamt-core: land #{NN} {slug} (…)` (drop `#{NN}` when grandfathered). Then run the **mandatory post-commit assertion** before the `git merge --squash`: `git show HEAD:proposals/archive/{resolved-filename}` must carry `Status: Implemented` + the `Validated …` footer, and `git status --porcelain` must be empty — halt (do **not** `git checkout` the base branch) on either failure, surfacing the verbatim diff. Only after the assertion passes: `git merge --squash` the proposal branch into the base branch as that one commit, then `git branch -D` the proposal branch **only after the merge commit succeeds**. Halt on any guard failure or git error.
