...
- **No `Target Features` work here.** This command writes the epic with the decomposition sections empty; `/p2-decompose-epic` fills them. Two reasons: keeps single-session sizing tight (Principle 1 — single-session sizing constraint), and keeps deep dialog at the right altitude per the stub-list-then-drill-in decomposition.
