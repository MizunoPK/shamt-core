...
5. **Zero matches** → halt; direct the user to `/pe1-define {slug}`.
...
2. Draft a proposed feature list. Each entry: **title** + **one-line goal**. Capture the list as a flat enumerated proposal — no nested sub-features, no per-feature deep dialog (that is `/pf1-define`'s job).
...
**Per-feature deep dialog is deferred to `/pf1-define`.** Do not start drafting per-feature goals, scope, success criteria, or stories here — that is the next altitude's work per the stub-list-then-drill-in decomposition.
...
```
/clear
/pf1-define {feature-slug-1}     # flesh out the first stub
/pf1-define {feature-slug-2}     # ...and so on
```

Each feature stub is **independently resumable**. The PO can drive `/pf1-define` on each stub in sequence, or run them in parallel by opening additional terminal tabs (the framework provides no runtime coordination machinery per the decomposition / parallelization step). Order is suggested by the `Recommended order` line just written to the epic.
