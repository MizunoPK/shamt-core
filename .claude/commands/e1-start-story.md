...
**Stub-aware merge (when Step 2 marked the invocation as stub-aware).** The existing ticket.md from /p4-decompose-feature already carries the scope one-liner in the body intake area; its parent feature/epic are the folder path, not headers. When present, a ## Decomposition Context breadth section is preserved verbatim — decomposition-owned breadth that seeds the dialog (Step 2); it is not rewritten. The open-questions dialog fleshes out the remaining template sections (Summary, Acceptance Criteria, etc.) — it does not rewrite the scope one-liner or the Decomposition Context.
