...
- **File exists, non-empty, and is NOT an f0 draft** — treat as re-entry. Read the file, confirm with the user whether to extend the existing proposal or start over. If extending, skip the template-seed step (Step-by-step Step 1 below) and resume drafting at Step-by-step Step 2; **if a prior Phase 2 validation footer is present, strip it before continuing — extending the proposal invalidates the prior validation, and Phase 2 must re-run on the extended content (the Step 6 exit gate enforces this).** If starting over, move the abandoned draft to `.shamt-core/proposals/archive/{slug}.draft-{timestamp}.md` (using `git mv` when tracked) before overwriting. The archive folder is the documented home for retired proposal files; the `.draft-{timestamp}` infix distinguishes abandoned drafts from implemented archives. For a *post-validation* scope increase that needs only an extra Proposed Changes row (not a re-draft), prefer the lighter **[In-place amendment](#in-place-amendment)** path below — strip the footer, append the row(s), and re-run `/validate-artifact`, with no full `/f1-propose-update` re-entry.
...
When a **downstream phase** of the framework-update flow (`/f2-plan-update-implementation`, `/f3-implement-update`) discovers that an already-validated proposal's **Proposed Changes** table is *missing* a row — a paired file that was overlooked, a reference site found only while planning or implementing, or a genuinely wanted change surfaced by the diff-coverage gate — it does **not** need a full `/f1-propose-update` re-run. The discovering command performs an **in-place amendment** instead:

1. **Strip** the prior `Validated …` footer from the proposal. Extending the change list invalidates the prior validation — the same rule the Slug-resolution re-entry note applies on extend. This strip step is load-bearing: `/validate-artifact` Step 8 only *appends* a footer, it never removes a stale one, so a site that says "re-validate" without "strip first" would leave a footer that lies.
2. **Append** the missing row(s) directly to the Proposed Changes table — the discovering command already knows the exact path + operation it found.
3. **Re-run** `/validate-artifact` on the whole proposal, then resume the halted phase.

The discovering command does the mechanical edit and then **halts for re-validation** — it does not proceed past an unvalidated proposal, so phase-per-command resumability ([Principle 1](../../../../templates/SHAMT_RULES.template.md#principle-1-phase-per-command--slug-resumability)) holds: re-validation remains its own command, and the footer keeps meaning "every row reviewed in one pass." Traceability rides on the existing footer convention; no separate amendment log is required.
