...
- **Footer contract.** The pragmatic Phase 9 rule is **subtree-level**: every path in master's sync set is master-owned (no per-file footer check). Any file the child adds under `.shamt-core/{templates,reference,host,scripts}/` outside that sync set is preserved with a warning. This is a tighter rule than the literal §4.7 wording proposed (per-file footer check), justified by the fact that most canonical files under `templates/` and `reference/` do not yet carry footers — applying the strict footer rule would leave the bulk of canonical content un-synced.
