...
- **Always-latest.** No version pinning. `import-shamt.sh` pulls master HEAD every invocation. Per §4.13, this is intentional — the framework's surface is small enough that a flag day isn't needed.
- **Self-updating.** `import-shamt.sh` is itself in the sync set. A new version overwrites the on-disk copy mid-run. The running script is already in memory and continues with the previous logic; the new version takes effect on the next invocation.
...
- **Footer contract.** The pragmatic footer-contract rule is **subtree-level**: every path in master's sync set is master-owned (no per-file footer check). Any file the child adds under `.shamt-core/{templates,reference,host,scripts}/` outside that sync set is preserved with a warning. This is a tighter rule than the literal §4.7 wording proposed (per-file footer check), justified by the fact that most canonical files under `templates/` and `reference/` do not yet carry footers — applying the strict footer rule would leave the bulk of canonical content un-synced.
