...
- Story folder resolves (via `stories/{slug}/` exact match or `stories/{slug}-*/` glob; halt on multiple or zero matches per the Global Story Invariants in [`SHAMT_RULES.template.md`](../../../../templates/SHAMT_RULES.template.md)).
