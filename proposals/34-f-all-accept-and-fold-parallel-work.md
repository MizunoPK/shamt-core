...
**This is the action-angle of a single gap.** Its sibling, `33-agents-trust-cross-session-provenance` (**now landed** — commit `0b04dd6`, archived under `proposals/archive/`; Principle 3 is live in `templates/SHAMT_RULES.template.md` and `CLAUDE.md`), attacked the **belief-angle** (an agent must not infer "I didn't see X this session" ⇒ "X never happened") by adding the new cross-cutting **Principle 3 — Disk-authoritative cross-session work**. The existing "never halt or revert on unrelated tree state" rules in `/f3` and `/f6` are the *action consequence* of that belief principle; this proposal extends that action consequence to the two flow-drivers, where it is currently missing. The two proposals were deliberately coordinated to land **separately, off the same files** (a D9 duplication-avoidance decision): `33` owned the rules-file principle (`templates/SHAMT_RULES.template.md`, `CLAUDE.md`); this proposal owns the driver-body action-rule (the four host `f-all`/`e-all` files). Because `33` has landed, this proposal's "per Principle 3" reference is now a **live** citation, not a forward reference.
...
Row count is **4** — well under the 10-op Phase-3 threshold, so no `/f2` implementation plan is required.
...
- **Dangling-reference risk (now LOW — `33` has landed)** — `33` (Principle 3) landed first (commit `0b04dd6`), so the "per Principle 3" citation is **live**, not dangling. The wording is nonetheless kept self-standing — anchored primarily on the **live** `/f3`/`/f6` accept-and-fold rules, with Principle 3 named as the *elevating* reference — so it remains correct and robust regardless of landing order (the rationale in Resolved Question 2).
...
- **Change-list completeness** — The paired edits are command ↔ SKILL.md (rows 1↔2, 3↔4). Confirm the **load-bearing** dispatch-prompt clause (Step 2/Step 3), not just the Notes bullet, is in scope for each command.
...
1. **Scope: `/f-all` only, or both `/f-all` and `/e-all`?** → **Both.** `/e-all` has the identical inline-phase dispatch topology and its `/e7` writes to `.shamt-core/proposals/`, carrying the same parallel-work loss exposure; same architecture, same exposure, preventive parity, and consistent with how the sibling `33` frames the gap ("both flow-drivers"). Rows 3–4 (the `/e-all` command + skill) are in scope.
2. **Principle-3 cross-reference wording.** → State the rule **self-standingly**, anchored on the **live** `/f3`/`/f6` accept-and-fold rules (which exist on master today), with Principle 3 named only as an *elevating* reference. **Update:** during this drafting `33` landed first (commit `0b04dd6`; Principle 3 now live in the rules file), so the citation is live rather than forward-looking — but the self-standing phrasing is retained so the rule is complete and correct regardless of landing order.
