...
- **define (stage 1)** — flesh out that file via the open-questions iterative dialog. The PO is a manager writing/polishing a ticket for an engineer: `/ps1-define` runs the dialog **plus a Pattern-1 validation loop**, producing a ready, validated planning `ticket.md`. Define is also runnable standalone (no prior draft) — it then seeds + fleshes from scratch the way `/p1-start-epic` / `/p3-start-feature` do today. **Only story-define folds the validation loop inline** — because the story ticket is the artifact handed directly to the Engineer flow and its `Validated …` footer is the very signal `/e1-start-story`'s pickup branch keys on. Epic- and feature-define (`/pe1-define` / `/pf1-define`) produce artifacts validated **separately** via `/validate-artifact` (exactly as `/p1`/`/p3` do today); they are decomposition inputs, not engineer-handoff artifacts, so they carry no inline loop.
...
| # | Canonical path | Operation | One-line change description |
|---|----------------|-----------|------------------------------|
| 21 | `shamt-core/host/templates/claude/commands/e1-start-story.md` | EDIT | Add the **ready-ticket pickup branch** by extending e1's existing Step-2 folder-path detection with the validation-footer signal (see Resolved Questions): a slug resolving to an existing nested `ticket.md` **with** a Pattern-1 `Validated …` footer → ready-ticket pickup (tracker fetch/reconcile + confirm, skip re-authoring); nested **without** footer → the existing bare-stub merge path; no nested folder → full freeform/tracker intake (ad-hoc story). No new command flag or template. Update `/p4`/`/p6` references to `/pf2-decompose` / `/ps0-draft`. |
| 22 | `shamt-core/host/templates/claude/skills/e1-start-story/SKILL.md` | EDIT | Mirror the e1 body changes. |
| 23 | `shamt-core/host/templates/claude/commands/e8-finalize-story.md` | EDIT | Update the two `/p5-finalize-epic` references (the epic-archive analogies, ~lines 70 & 92) → `/pe3-finalize`. (e8 contains no `/p4`/`/p6` references; the Tech-Stories story-archive logic itself is unaffected by the renames.) |
...
| 26 | `shamt-core/templates/SHAMT_RULES.template.md` | EDIT | Update the PO-flow overview (line 19 — `/p2`/`/p3`/`/p4` → grid names), the Finalize-commands section (`/p5-finalize-epic` → `/pe3-finalize`, line 138), §Standing Tech Stories epic (lines 167/169 — `/p1-start-epic` → `/pe1-define`, seed entry `/p6-draft-tech-story` → `/ps0-draft`), and §"Stub IDs are preserved" (line 404 — producer commands `/p2`/`/p3`/`/p4` renamed). |
...
| 32 | `shamt-core/templates/epic.template.md` | EDIT | Update owner references — `/p1-start-epic` → `/pe1-define` (lines 26, 40, 48) and `/p2-decompose-epic` → `/pe2-decompose` (lines 40, 48); document the stage-0 draft-capture convention (Scratch Notes + draft status, ingested by `/pe1-define`). |
| 33 | `shamt-core/templates/feature.template.md` | EDIT | Update owner references — `/p2-decompose-epic` → `/pe2-decompose` (line 40), `/p3-start-feature` → `/pf1-define` (lines 26, 40, 50, 58), and `/p4-decompose-feature` → `/pf2-decompose` (lines 40, 50, 58); document the stage-0 draft-capture convention. |
...
### F. Install / sync script seed text

These scripts emit child-facing Tech Stories epic/feature scaffolding that hardcodes the deleted `/p6-draft-tech-story` (rules §Standing Tech Stories epic confirms the standing epic is seeded by `init-shamt.sh` / `import-shamt.sh`). Each must be updated to `/ps0-draft` or a newly scaffolded/synced child gains a dangling reference to a non-existent command.

| # | Canonical path | Operation | One-line change description |
|---|----------------|-----------|------------------------------|
| 36 | `shamt-core/init-shamt.sh` | EDIT | In the seeded Tech Stories `epic.md` / `feature.md` heredocs (around lines 515, 531, 544): `/p6-draft-tech-story [bugs\|quick-wins]` → `/ps0-draft [bugs\|quick-wins]`. |
| 37 | `shamt-core/import-shamt.sh` | EDIT | In the seeded Tech Stories `epic.md` / `feature.md` heredocs (around lines 385, 397): `/p6-draft-tech-story [bugs\|quick-wins]` → `/ps0-draft [bugs\|quick-wins]`. |
...
- **Engineer-boundary regression** — the new `/e1-start-story` ready-ticket pickup branch must not break the existing freeform/tracker intake for ad-hoc stories. Misdetection (treating an ad-hoc ticket as a ready planning ticket, or vice-versa) would skip or duplicate intake. **Mitigation — the three states are distinguishable by the validation-footer signal** (resolved below): nested `ticket.md` with a `Validated …` footer = ready ticket; nested without footer = bare stub; unresolved/non-nested = ad-hoc.
...
- **Cross-reference completeness** — `/p1`–`/p6` are referenced across README, the rules template, model_selection, all four tracker profiles, epic/feature/ticket templates, the e1/e8 bodies, **and the `init-shamt.sh` / `import-shamt.sh` Tech Stories seed heredocs** (which hardcode `/p6-draft-tech-story`). A missed reference leaves a dangling command name (D4 reference-validity audit finding).
...
- **Affected surfaces** — commands, skills, references (trackers + model_selection), templates (epic/feature/ticket), rules, README, **and the `init-shamt.sh` / `import-shamt.sh` Tech Stories seed heredocs** (section F — they hardcode `/p6-draft-tech-story`). PO altitude only on the producer side; one consumer-side touch (e1/e8). No statusline impact (statusline reads pointers, not command names — verified).
- **Propagation plan** — requires `/f4-regen-framework` + child `import-shamt`. Phase 3 (`/f2-plan-update-implementation`) is **required** (row count 37 > 10) before `/f3-implement-update`.
...
- ~~Q: How does `/ps1-define` relate to `/e1-start-story`?~~ → A: `/ps1-define` is the manager polishing the ticket — open-questions dialog **+ a Pattern-1 validation loop** producing a ready, validated planning ticket. `/e1-start-story` stays as the Engineer-flow entry. **Why only story-define validates inline** (and not `/pe1-define` / `/pf1-define`): the story ticket is the only PO artifact handed *directly* to an engineer, and its validation footer doubles as the e1 readiness signal — so the loop is folded in. Epics/features remain decomposition inputs validated via standalone `/validate-artifact`, unchanged from `/p1`/`/p3` today.
...
- ~~Q: (derived) How does `/e1-start-story` distinguish a `/ps1-define`-ready ticket from a bare stub vs. an ad-hoc story (the misdetection risk)?~~ → A: Extend e1's **existing** folder-path detection (it already inspects nested parentage to spot stubs) with the **validation-footer** signal: a resolved nested `ticket.md` carrying a Pattern-1 `Validated …` footer = ready-ticket pickup (tracker fetch/reconcile + confirm); nested *without* a footer = the existing bare-stub merge; no nested folder resolved = full ad-hoc intake. The footer already exists — it is exactly what `/ps1-define`'s validation loop stamps — so **no new command flag, status marker, or template** is introduced, consistent with the "no new command flag and no new template" detection design e1 already uses for stubs.

---
Validated 2026-06-14 — 7 rounds, 1 adversarial sub-agent confirmed (3 adversarial passes total; the first two each surfaced one issue and reset the counter)
