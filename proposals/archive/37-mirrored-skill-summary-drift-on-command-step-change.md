...
- **`/f5-audit-framework` D2** already names the contract — its Step 2 D2 check at `host/templates/claude/commands/f5-audit-framework.md:98` reads "Step count and order match (or the body explicitly says 'Summary: see canonical command body' and the summary is faithful)" — so **detection exists post-hoc and worked here.** The gap is upstream **prevention**, not detection. That same f5 line, however, is what *blesses* a faithful Summary paraphrase, so the forward guard must flip it too (row 6) — leaving it would keep the audit's own D2 check licensing the very paraphrase the new pointer rule forbids, a fresh D2/D9 contradiction against `reference/audit_dimensions.md` (row 1).

The fix has two layers (scope confirmed in the dialog: take on the structural change here, not as a follow-up). **Structurally** — eliminate the drift class at its source by **removing the hand-written step paraphrase from every SKILL.md**: each `## Protocol` section keeps only the canonical "follow the command body verbatim — see `commands/{name}.md`" pointer the files already carry, and drops the numbered "Summary:" list (the audit's D2 already blesses this exact pointer form). With nothing duplicated, nothing can drift; no regen-script change is needed (regen keeps copying `SKILL.md` verbatim). **As a forward guard** — flip the now-uncodified contract into an explicit rule: a SKILL `## Protocol` **is** the command-body pointer and **must not** re-introduce a step paraphrase. Name that rule in `reference/audit_dimensions.md` (where D2's definition lives), flip the **operative** D2 check in `host/templates/claude/commands/f5-audit-framework.md` (the line the audit actually runs, today blessing a faithful Summary) so re-introduction is caught post-hoc, and reference the rule from authoring (`/f1`), validation (`/validate-artifact`), and the proposal template — the three sites that today only softly gesture at the mirror. The `/f3` diff-coverage gate and the regen script are deliberately **not** touched (the diagnosis confirmed each is correct as-is).

---

## Proposed Changes

> **Phase 3 required — file count 38 > 10. Run /f2-plan-update-implementation 37-mirrored-skill-summary-drift-on-command-step-change before /f3-implement-update.** The change set is one large mechanical migration (33 SKILL.md edits, row 5) plus five small forward-guard edits (rows 1–4, 6). The migration's identical-edit rows are enumerated by the Phase 3 plan; row 5 names the full set + the exact mechanical edit + the whole-plan acceptance invariant.
...
| 6 | `shamt-core/host/templates/claude/commands/f5-audit-framework.md` | EDIT | Step 2's D2 check (line ~98): replace the "Step count and order match (or the body explicitly says 'Summary: see canonical command body' and the summary is faithful)" allowance with the **pointer-form rule** — a `skills/{name}/SKILL.md` `## Protocol` **is** the command-body pointer and must not carry a numbered step paraphrase; flag any SKILL `## Protocol` that re-introduces a step list as a D2 finding. Keeps the running audit check consistent with the row-1 `reference/audit_dimensions.md` definition (else f5 would still bless the paraphrase the new rule forbids — a D2/D9 self-contradiction). Reference the pointer rule. |

**Path discipline:** all canonical (`reference/`, `host/templates/claude/commands/`, `host/templates/claude/skills/`, `proposals/`). No `.claude/` paths — regen (Phase 5) propagates the 33 SKILL edits + the 3 command-body edits (`/f1`, `/validate-artifact`, `/f5-audit-framework`) into `.claude/`.
...
- **Drift risk** — row 5 (33 skills) + rows 2–3, 6 (three command bodies — `/f1`, `/validate-artifact`, `/f5-audit-framework`) regenerate `.claude/` copies via `/f4-regen-framework`; skipping regen leaves a large generated/canonical mismatch. Standard regen covers it; `--check` zero-drift confirms. (Row 1 reference doc and row 4 proposal template are not host-templated — no `.claude/` output.)
...
- **Child-project compatibility** — children pick up the 33 regenerated skills + the three command bodies + the template on next `import-shamt`; no artifact migration (this changes generated-doc content, not any on-disk work-artifact shape).
...
1. `git revert <commit-sha>` on the canonical edit commit (reverts all 38 file edits atomically — the proposal lands as one squash commit).
2. Run `/f4-regen-framework` to propagate the revert into `.claude/` (the 33 skills + the three command bodies).
...
- **Change-list completeness** — confirm row 5's set is exactly the **33** command-backed skills (a `skills/{name}/` with a matching `commands/{name}.md`); confirm no command-backed SKILL is omitted and no non-command skill is wrongly included. Confirm rows 1–4, 6 are mutually reinforcing (the reference defines the pointer rule (row 1); the audit's *running* D2 check (row 6, `f5-audit-framework.md:98`) enforces it; `/f1`, `/validate-artifact`, and the template each reference it). The "summary is faithful" allowance lives only at `f5-audit-framework.md:98` — row 6 is the mirrored canonical edit that keeps the running audit consistent with the row-1 definition (omitting it would re-create exactly the missed-mirror class #37 exists to prevent).
...
- **Affected surfaces** — one reference doc (`audit_dimensions.md`), three command bodies (`/f1`, `/validate-artifact`, `/f5-audit-framework`) + their regenerated skills, the proposal template, and **all 33** SKILL files + their regenerated `.claude/` copies. Cross-doc consistency: the pointer-rule wording in `reference/audit_dimensions.md` reads parallel to the running D2 check in `f5-audit-framework.md` (row 6) it codifies; the `/f1` + `/validate-artifact` references point at it by name.
- **Propagation plan** — requires `/f4-regen-framework` (33 skills + three command bodies) + child `import-shamt`. **Phase 3 required (38 file ops > 10).**
