...
|---|----------------|-----------|------------------------------|
| 1 | `shamt-core/host/templates/claude/commands/sync-proposals.md` | EDIT | Front-matter `description:` — replace "prints one copy-paste block per proposal … for manual copy-paste" with the new always-local behavior: direct-write each proposal into the local master's `proposals/incoming/`, and halt with actionable guidance when `master_url` is not a usable local path (e.g. a git URL). No copy-paste blocks. |
...
- **Propagation plan** — Requires regen + child import to take effect; the change is purely behavioral and is picked up on the next `import-shamt`. A child whose `master_url` is a **local path** needs no manual nudge — it transparently switches from copy-paste to direct-write. For a child whose `master_url` is a **git URL** this is a **breaking** change: `/sync-proposals` no longer emits copy-paste blocks but halts at the Step 1.5 guard (it does **not** "keep copy-paste"), so that child must set a local `master_url` or copy proposals upstream by hand — cross-reference the "Git-URL master regression" risk above.
...
---
Validated 2026-06-20 — 4 rounds, 1 adversarial sub-agent confirmed (/f-all Phase 2)
