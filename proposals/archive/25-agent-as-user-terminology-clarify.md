...
Proposal #21 standardized the required Phase-5 run as **"agent-as-user execution"** — the term now used in the rules, the `user-simulator` persona, `reference/testing.md`, and even the *Purpose* paragraph of `templates/testing_standards.template.md` itself (lines 22, 31: "to drive the per-story agent-as-user execution", "Phase 5 runs the agent-as-user execution only"). But the **driving-procedure source** that this run reads — the section of `TESTING_STANDARDS.md` that documents *how* to drive the project as a user — is still labeled **"Manual-as-user testing"** in five coupled instruction-path spots:
...
- `init-shamt.sh:594` — the root-level installer's init completion-prompt heredoc instructs a fresh agent to "write the **manual-as-user** driving procedures" when filling in `TESTING_STANDARDS.md` at install time (descriptive prose in a one-time prompt, not a verbatim-matched section name).
...
| 4 | `shamt-core/init-shamt.sh` | EDIT | In the init completion-prompt heredoc (line 594), "write the manual-as-user driving procedures" → "write the agent-as-user driving procedures", keeping the install-time instruction aligned with the renamed section. |

**Path discipline:** all four are EDITs to canonical sources (`templates/`, `host/templates/claude/`, `reference/`, and the root-level `init-shamt.sh` installer). No `.claude/` paths; no CREATE/DELETE/MOVE. 4 rows ≤ 10 — no Phase 3 plan required.
...
- **Init-prompt scope (change #4)** — the `init-shamt.sh:594` edit affects only **future** `init-shamt.sh` runs (new child installs); the completion prompt is a one-time, install-time instruction, not a regenerated `.claude/` artifact, so already-initialized children are unaffected by it. The wording is descriptive prose, not a verbatim-matched section name, so even a stale copy would not break the `user-simulator` pre-flight grep (unlike the load-bearing heading↔persona pairing). Lowest-risk of the four rows; included for terminology-sweep completeness so the post-edit zero-hits grep is honest.
...
- **Change-list completeness** — the easy-to-forget pairing is heading (#1) ↔ persona verbatim reference (#2): both must move together. Also confirm the grep is exhaustive — a fresh `grep -rni "manual-as-user"` across the live canonical surface (`templates/ reference/ host/templates/claude/ scripts/ init-shamt.sh CLAUDE.md README.md` — note the root-level `init-shamt.sh` must be included) should return **zero** hits after the edits (the five current hits map to the four rows, with #1 covering two sites in one file). Out of scope for the zero-hits assertion: `proposals/archive/` (archived #21 legitimately retains the old term as historical record — do **not** edit it) and generated `.claude/agents/user-simulator.md` (cleared by the subsequent `/f4-regen-framework`).
...
- **Rollback feasibility** — trivial; four file EDITs (five textual sites — row #1 covers two), fully reverted by `git revert` + regen. No DELETE/MOVE, no lost history.
- **Affected surfaces** — templates, references, personas (via `host/templates/claude/agents/`), and the root-level `init-shamt.sh` installer script (its one-time install-time completion prompt). No rules, commands, or skills touched. Cross-doc consistency to verify: the renamed heading appears identically in the template heading and the persona's quoted section name.
...
- ~~Q: #21 names the Phase-5 run "agent-as-user execution" but the procedure-source section is labeled "Manual-as-user testing" (colliding with the human-only `manual_test_plan.md`). Rename, add a clarifier, or reject as intentional?~~ → A: **Rename** "Manual-as-user" → "Agent-as-user" across all five spots (heading + verbatim persona reference + reference doc + update-trigger + `init-shamt.sh` install prompt). Aligns with #21's established term, removes the human-manual-test collision, and fixes the template's own internal inconsistency (its Purpose already says "agent-as-user").
