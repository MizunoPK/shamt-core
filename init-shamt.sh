...
#   6. Render the child's CLAUDE.md (from templates/SHAMT_RULES.template.md) at
#      <target>/ — always overwritten, since the root rules file is fully
#      Shamt-managed — then append a managed .gitignore block ignoring Shamt's
#      generated footprint (/.shamt-core/, /.claude/, and /CLAUDE.md — always);
...
if [ "$SELF_HOST" -eq 0 ]; then
  ...
  if [ -f "$rules_src" ]; then
    ...
    log "  rendered CLAUDE.md (from SHAMT_RULES.template.md — always overwritten)"
  else
    warn "SHAMT_RULES.template.md not found at $rules_src — CLAUDE.md not rendered."
  fi

  # ---- gitignore Shamt-generated content (child only) ----
  # The child repo should not track Shamt's generated footprint: .claude/ is
  # regenerated by /f4-regen-framework, .shamt-core/ is re-pulled by import-shamt,
  # and /CLAUDE.md is re-rendered from the rules template on every regen — all
  # three are re-derivable. Append an idempotent managed block to the child's root
  # .gitignore. /CLAUDE.md is ALWAYS ignored: the root rules file is fully
  # Shamt-managed (always overwritten), not the child's own file.
  # Root-anchored patterns match only the install footprint.
  gi="$TARGET_DIR/.gitignore"
  if [ -f "$gi" ] && grep -q '# >>> Shamt (managed)' "$gi"; then
    log "  .gitignore already carries the Shamt block — leaving it."
  else
    {
      printf '\n# >>> Shamt (managed) — generated/regenerated content; not tracked in this repo.\n'
      printf '#     Restore .shamt-core/ via import-shamt; regenerate .claude/ via /f4-regen-framework.\n'
      printf '/.shamt-core/\n'
      printf '/.claude/\n'
      printf '/CLAUDE.md\n'
      printf '# <<< Shamt\n'
    } >> "$gi"
    log "  updated .gitignore (ignoring /.shamt-core/, /.claude/, /CLAUDE.md)"
  fi
