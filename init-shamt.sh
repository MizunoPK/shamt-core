...
if [ "$SELF_HOST" -eq 0 ]; then
  ...
  # regenerated by /f4-regen-framework and .shamt-core/ is re-pulled by
  # import-shamt, so both are re-derivable. Append an idempotent managed block to
  # the child's root .gitignore. /CLAUDE.md is listed ONLY when we seeded it (a
  # full Shamt render); a pre-existing child CLAUDE.md is the child's own file and
  # is never ignored. Root-anchored patterns match only the install footprint.
  ...
  if [ -f "$gi" ] && grep -q '# >>> Shamt (managed)' "$gi"; then
  ...
  else
    {
      ...
      printf '/.shamt-core/\n'
      printf '/.claude/\n'
      [ "$CLAUDE_SEEDED" -eq 1 ] && printf '/CLAUDE.md\n'
      printf '# <<< Shamt\n'
    } >> "$gi"
    log "  updated .gitignore (ignoring /.shamt-core/, /.claude/$( [ "$CLAUDE_SEEDED" -eq 1 ] && printf ', /CLAUDE.md' ))"
  fi
