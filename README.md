...
| `/f-all {slug}` | Meta-driver (spans Phases 2–7, not a numbered phase) — walk a proposal through every remaining phase end-to-end (validate → optional `/f2`+plan-validate → `/f3` → `/f4` → `/f6`; `/f5` excluded) by dispatching one independent agent per phase, deriving the start phase from on-disk artifacts, and pausing only on a structured open question or halting on any other non-clean outcome. **Master / self-host only** — in a child it halts and redirects to the per-phase commands | shipped |
