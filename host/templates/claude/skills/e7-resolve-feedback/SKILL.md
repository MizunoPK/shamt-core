...
5. **Root-cause / upstream proposals** — generalizable patterns → `.shamt-core/proposals/<proposal-slug>.md` (descriptive slug, not the story slug) via the framework-update flow. Story-specific patterns stay in-story. A bug-originated proposal goes through `/f1-propose-update`'s incident-origin adversarial root-cause diagnosis (Step 2.5); the Step 2 phase-attributed root cause is the seed it distrusts, not the final word.
