...
2. **Resolve `{id-or-slug}`** — resolve per `templates/SHAMT_RULES.template.md` §PO-tree resolution (tree-wide feature glob) and legacy-flat fallback. Halt on multiple matches. A new feature gets an allocated ticket ID; a Mode A stub preserves its ID. On a one-match resolution, **inspect `feature.md`** to decide Mode A vs. re-entry:
   ...
   - Depth sections drafted (a prior `/p3-start-feature` deep-dive ran) → re-entry. Confirm refetch / overwrite / extend / exit. **Preserve `**Ticket ID:**` header verbatim** regardless of branch; strip any prior `Validated …` footer when extending. (Parentage is the folder path.)
3. **For new features** (zero matches): ask the user for a 2–4-word brief description; propose and create `epics/{parent-epic-folder}/features/{ID}-{slug}-{brief}/`. Mode B nests under Tech Stories epic; Mode C nests under matched or Tech Stories epic depending on tracker + slug shape.
...
8. **Suggest** `/clear` + `/validate-artifact epics/{parent-epic-folder}/features/{ID}-{slug}-{brief}/feature.md` as the next command. Do **not** auto-invoke — per [Principle 1](../../../../../templates/SHAMT_RULES.template.md#principle-1-phase-per-command--slug-resumability), every command stays independently runnable.
