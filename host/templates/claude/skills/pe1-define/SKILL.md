...
2. **Resolve `{id-or-slug}`** — a ticket ID globs `epics/{ID}-*/`; a slug tries `epics/{slug}/` (exact) then **both** `epics/{slug}-*/` and `epics/*-{slug}-*/` (glob; epics are top-level; see §PO-tree resolution). Halt on multiple matches. On re-entry to a populated `epic.md`, confirm extend / overwrite / exit; **strip any prior `Validated …` footer if extending**.
3. **For new epics:** allocate a ticket ID `T{N}` (max across `epics/`, `features/`, `stories/` + 1), ask for a 2–4-word brief description, and create `epics/{ID}-{slug}-{brief}/` with the `**Ticket ID:** T{N}` header. Before exiting, write the resolved epic-folder path to `shamt-state/active-epic` (create `shamt-state/` if absent) — the status-line active-epic pointer.
