...
2. **D2 cross-doc consistency** — for every pattern / invariant in `templates/SHAMT_RULES.template.md`, confirm matching skill/command/persona bodies describe the same steps, exit criteria, and naming (including **write-path placeholder vocabulary**: a PO-tree producer diverging from §PO-tree resolution's canonical `{epic-folder}` / `{feature-folder}` full nested write-form `epics/{epic-folder}/features/{feature-folder}/stories/…` — e.g. writing the collapsed `epics/{parent-feature-folder}/stories/…` — is a **HIGH** D2 finding, vertical rules↔host not D7).
...
6. **D6 project-doc currency** — `.shamt-core/project-specific-files/ARCHITECTURE.md` + `.shamt-core/project-specific-files/CODING_STANDARDS.md` + `.shamt-core/project-specific-files/TESTING_STANDARDS.md` present in the child project; `Last Updated` within `doc_staleness_threshold_days` (default 60 per `.shamt-core/shamt-config.json`) — header contract per the three project-doc templates (`architecture.template.md` / `coding_standards.template.md` / `testing_standards.template.md`). Missing required doc = HIGH; stale doc = MEDIUM; all-three-missing not-applicable case for in-development `shamt-core/` = single LOW informational.
7. **D7 terminology consistency** — one canonical term per concept across all canonical docs ("Quick"/"Standard" never "Small"/"Full", "Build" never "Implement"). Non-canonical synonym in an instruction path = HIGH. **Carve-out:** path-template placeholders pinned by the rules file's §PO-tree write-vocabulary (`{epic-folder}` / `{feature-folder}`) are checked by **D2 (rules ↔ host), not D7**.
