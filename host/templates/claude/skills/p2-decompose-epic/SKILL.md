...
- **Decomposition exit gate ≠ `/validate-artifact`.** Per INFRASTRUCTURE.md §2.1 epic-level review row, the gate is a 2-condition stub-batch check run **before** stubs land on disk. `/validate-artifact` runs the full Pattern 1 loop against `epic.md` (already, before this command) and against each `feature.md` (later, after `/p3-start-feature` completes its dialog). Do not conflate.
