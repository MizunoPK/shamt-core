...
1. **Resolve `{id-or-slug}`** — a ticket ID globs `epics/{ID}-*/`; a slug tries `epics/{slug}/` (exact) then **both** `epics/{slug}-*/` and `epics/*-{slug}-*/` (glob). Halt if missing — direct to `/p1-start-epic {slug}`. Each New feature gets an allocated ticket ID (`T{N}`); the parent epic is determined by the nested folder path.
