...
Phase 6  /f5-audit-framework                                   [always]
Phase 7  /f6-archive-proposal                                  [always]
```

**`/f5-audit-framework` (Phase 6) is included in the chain**, always-on, between `/f4` (Phase 5, regen) and `/f6` (Phase 7, archive) — the literal Phase 5→6→7 order, which is the normal framework-update sequence. After a change is implemented and regenerated, `/f-all` runs the audit so the proposal's landing is automatically verified against the twelve audit dimensions, and the audit's auto-fixes and any this-run f0 captures **fold into the `/f6` squash commit** alongside the proposal + regen output (the fold-into-`/f6` behavior `f5-audit-framework.md` Step 3's "In-flow logging" already specifies). The audit slots into `/f-all`'s **existing driver-lifted-inner-persona topology** (identical in shape to Phase 2 inline-validation, exactly one nesting level): the dispatched audit phase agent runs the audit's inline Steps 1–4 to a clean round and **halts before Step 5 without invoking `/f5-audit-framework`**; the **driver** then lifts the `audit-checker` confirmation up to itself. The audit's only sub-agent is that `audit-checker` clean-round confirmation — its sweep / auto-fix / f0-capture are all inline primary work, not fan-out, and f0-capture is an inline stub-write (it applies `/f0-draft-proposal`'s logic directly, spawning nothing). See the **Phase 6 — `/f5-audit-framework`** dispatch subsection under Step 2 for the full mechanics.
...
| `regenerate-framework.sh --check` reports zero drift (and branch exists) | `/f4` done (or `/f5` done — see note) | Phase 6 (audit) |
| Proposal moved under `proposals/archive/` | `/f6` done | Nothing — report already-complete and exit |

**Resumability after Phase 6 relies on the audit's idempotency, not a new disk signal.** The audit writes **no new disk artifact** to mark "Phase 6 done," so there is no new discriminator row — the `regen --check zero drift + branch exists` row above routes to **Phase 6 (audit)** (the audit is the first thing the post-regen state routes to). A resumed run therefore re-enters and **re-runs the ENTIRE audit phase** — both the primary's inline Steps 1–4 loop **and** the driver-lifted `audit-checker` confirmation (resume does **not** skip the checker; with no disk marker the driver cannot tell a clean post-audit state from a clean post-`/f4` state, so it re-derives Phase 6 and re-runs the whole phase). On an already-clean framework both converge immediately: the primary's first round is clean (no new auto-fix and no new f0 draft) and `audit-checker` confirms with nothing resettable, so the harmless re-run advances to `/f6`. A clean post-audit state and a clean post-`/f4` state are *indistinguishable on disk* and both correctly route to Phase 6 — intentional and safe.
...
**Phase 6 — `/f5-audit-framework`.** Inline-validation topology — the **same** driver-lifted-inner-persona shape as Phase 2, applied to the audit (exactly one nesting level). The audit phase agent **runs f5's inline Steps 1–4 directly** (sweep D1, then D2–D12; auto-fix simple findings in place, f0-capture intricate ones; evaluate the round) and **LOOPS those steps itself until a clean round** — *clean* = the f5 adapted condition, **no new auto-fix AND no new f0 draft** (a clean round may still report already-captured intricate findings). **f0-capture is an inline file-write the audit primary performs itself** — it applies `/f0-draft-proposal`'s stub-writing *logic* directly (writes the `proposals/{slug}.md` f0 stub with the f0 status banner), **NOT** a `/f0-draft-proposal` sub-agent dispatch (`/f0-draft-proposal` spawns nothing — it is a pure stub-writer), so capture adds **no** nesting level. The audit phase's **only** sub-agent is the driver-lifted `audit-checker`. On reaching a clean round the agent **halts BEFORE f5's Step 5** and returns to the driver — it must **NOT** invoke `/f5-audit-framework`, since that would auto-spawn `audit-checker` (a forbidden 2nd nesting level), exactly as Phase 2's primary must not invoke `/validate-artifact`. The **driver** then dispatches `audit-checker` (Haiku) itself against the canonical surface, passing it the target context, the dimension list, and the findings captured this run (per f5 Step 5). On any **resettable** finding from the checker (a new simple finding or genuinely-uncaptured intricate finding — even one LOW; an already-captured intricate finding does **not** reset), the driver **re-dispatches the audit primary** to handle it (auto-fix / capture) and re-converge to a clean round, then **re-runs `audit-checker`**, repeating until the checker replies `CONFIRMED: Zero issues found after adversarial review.`. No persona edit — `audit-checker` is reused unchanged, lifted by the driver the same way `validation-checker` is. The audit's auto-fixes **and** this-run f0 captures **fold into the `/f6` squash commit** (not a separate commit) — `f5-audit-framework.md` Step 3's "In-flow logging" already specifies this fold-into-`/f6` behavior; `/f-all` simply realizes that slot. The audit never prompts the user (it captures rather than halting), so this phase has **no** open-question pause sentinel — it advances on the `CONFIRMED` reply or halts on non-convergence (the existing strict-halt path).

**Phase 7 — `/f6-archive-proposal`.** Self-contained — dispatch one `Agent` to run `/f6-archive-proposal {slug}`. **Before this autonomous archive** (an irreversible squash-merge + branch delete), the driver re-confirms `/f6`'s own pre-merge guards are evaluated, not bypassed: instruct the dispatched agent to evaluate every pre-merge guard and **halt without merging** on any failure (HEAD on the proposal branch; working tree carries the canonical edits + archive move + `.claude/` regen output + the Phase-6 audit's auto-fixes and this-run f0 captures; `/f4` and `/f5` have run; the proposal branch is a clean descendant of the base branch). The Phase-6 audit auto-fixes (and their regen output) and this-run f0 captures **fold into this one `/f6` squash commit** alongside the proposal + regen output — no separate commit. Naming *what* must be in the working tree is not enough: instruct the dispatched agent to **`git add -A` before the proposal-branch commit** so the regen output, audit auto-fixes, and f0 captures (including untracked files, and the validated content modification the Step 2 `git mv` leaves unstaged) actually land in the squash commit rather than being silently dropped — the staging *mechanism*, not just the contents. `/f-all` never bypasses these guards.
...
- `/f5-audit-framework` **is** run as Phase 6 of the chain (between `/f4` and `/f6`), via the driver-lifted `audit-checker` topology; its auto-fixes + this-run f0 captures fold into the `/f6` squash commit.
